...
if [ "$GCINCO" = true ]
	then
	cmake -DCMAKE_CXX_COMPILER='g++-5' -B. -DTEST=$TEST_PARAM -DCOVERAGE=$COVERAGE_PARAM -DDOXYDOC=$DOXY_DOC_PARAM 	-DCMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE ../project/  && make VERBOSE=1
else
	cmake -DTEST=$TEST_PARAM -DCOVERAGE=$COVERAGE_PARAM -DDOXYDOC=$DOXY_DOC_PARAM -DCMAKE_BUILD_TYPE=$CMAKE_BUILD_TYPE ../project/  && make VERBOSE=1
fi
