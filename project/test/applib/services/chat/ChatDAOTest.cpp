...
TEST_F(ChatDAOTest, saveMSGToDBAndRetrieve) {
	...
	bool resultado = dao.saveMsgFromTo(mensaje, "joni", "matias");
	ASSERT_TRUE(resultado);

	std::string retrieved;
	bool resultado2 = dao.getMsgBetween("joni", "matias", retrieved);
	...
	//Limpio la DB
	connector.deleteKey("joni-matias");
}
...
TEST_F(ChatDAOTest, retrieveMultipleMsgs) {
	...
	bool resultado = dao.saveMsgFromTo(mensaje1, "joni", "matias");
	ASSERT_TRUE(resultado);
	string mensaje2 = "chau gato";
	resultado = dao.saveMsgFromTo(mensaje2, "joni", "matias");
	ASSERT_TRUE(resultado);
